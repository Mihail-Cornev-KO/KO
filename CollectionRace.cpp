...
void CrRandomTick()
{
	while (true)
	{
		...
		//printf("CrRandomTick is working -1\n");
		if (Engine != NULL)
		{
			//printf("CrRandomTick is working 0\n");
			if (!Engine->IsCRActive)
				continue;

			//printf("CrRandomTick is working 1\n");
			if (Engine->CrRandomReward > 0)
			{
				//printf("CrRandomTick is working 2\n");
				if (Engine->uiCollection->minimaliaze)
					continue;

				//printf("CrRandomTick is working 3\n");
				for (int i = 0; i < 10; i++)
				{
					//printf("CrRandomTick is working 4\n");
					if (Engine->pCollectionRaceEvent.isRandom[i] > 0)
					{
						//printf("CrRandomTick is working 5\n");
						Engine->uiCollection->LoadItems3(Engine->pCollectionRaceEvent.RandomID[i], Engine->pCollectionRaceEvent.RandomSlotID[i], Engine->pCollectionRaceEvent.RewardsItemRate[i]);
					}
				}

				Engine->CrRandomRewardsize++;
			}
		}
	}
}
...
CUICollectionRacePlug::CUICollectionRacePlug()
{
	...
	minimaliaze = false;
	vector<int>offsets;
	offsets.push_back(0x55C);  // co_battle_score Satýr : 250
	offsets.push_back(0);
	...
	ParseUIElements();
	InitReceiveMessage();
	CreateThread(NULL, NULL, LPTHREAD_START_ROUTINE(CrRandomTick), NULL, 0, 0);
}
...
void CUICollectionRacePlug::LoadItems3(uint8 randomid, uint8 slotid, uint16 nRate)
{
	if (!randomid)
	{
		//printf("CrRandomTick is working 61\n");
		return;
	}

	if (!nRate)
	{
		//printf("CrRandomTick is working 63\n");
		return;
	}

	if (Engine->CrRandomItem[randomid].size() <= 0)
	{
		//printf("CrRandomTick is working 64\n");
		return;
	}

	if (Engine->CrRandomRewardsize >= Engine->CrRandomItem[randomid].size())
	{
		printf("CrRandomTick is working 7 | Engine->CrRandomItem[randomid].size(): %d \n", Engine->CrRandomItem[randomid].size());
		Engine->CrRandomRewardsize = 1;
	}

	uint32 n_randomItem = Engine->CrRandomItem[randomid][Engine->CrRandomRewardsize];
	//m_Items[slotid].ItemID = n_randomItem;

	printf("CrRandomTick is working Engine->CrRandomItem[randomid][Engine->CrRandomRewardsize]: %d || randomid: %d  || Engine->CrRandomRewardsize: %d || n_randomItem: %d \n",
		Engine->CrRandomItem[randomid][Engine->CrRandomRewardsize], randomid, Engine->CrRandomRewardsize, n_randomItem);
	TABLE_ITEM_BASIC* tbl = Engine->tblMgr->getItemData(n_randomItem);
	if (tbl != nullptr)
	{
		//printf("CrRandomTick is working 8\n");
		m_Items[slotid].tbl = tbl;
		Engine->SetString(m_Items[slotid].txt_return_first, tbl->strName);
		//Engine->SetString(m_Items[slotid].txt_return_second, to_string(1));
		//float per = (float)((float)nRate / (float)100) > 100 ? 100 : (float)((float)nRate / (float)100);
		//Engine->SetString(m_Items[slotid].txt_rate, string_format(xorstr("%.2lf%%"), per));

		if (TABLE_ITEM_EXT* ext = Engine->tblMgr->getExtData(tbl->extNum, n_randomItem))
		{
			...
			Engine->SetTexImage(m_Items[slotid].area_return, dxt);
		}
		else {
			...
			Engine->SetTexImage(m_Items[slotid].area_return, szIconFN);
		}

		Engine->SetState(m_Items[slotid].btn_return, eUI_STATE::UI_STATE_BUTTON_ON);
	}
	else
	{
	//	printf("CrRandomTick is working 9\n");
		m_Items[slotid].ItemID = m_Items[slotid].ItemCount = 0;
		m_Items[slotid].tbl = nullptr;
		Engine->SetString(m_Items[slotid].txt_return_first, xorstr(""));
		Engine->SetString(m_Items[slotid].txt_return_second, xorstr(""));
		Engine->SetString(m_Items[slotid].txt_rate, xorstr(""));
		Engine->SetVisible(requital[slotid], false);
	}
}
...
void CUICollectionRacePlug::Start()
{
	...
	//Engine->WriteChatAddInfo(D3DCOLOR_ARGB(255, 197, 209, 189), false, string_format("Zone : %s - Zoneid : %d", GetZoneName(Engine->pCollectionRaceEvent.sZone).c_str(), Engine->pCollectionRaceEvent.sZone).c_str());
	uint8 baseCount = 0;
...
void CUICollectionRacePlug::LoadItems(uint32 Items[10], uint32 Counts[10], uint8 nRate[10])
{
	for (int i = 0; i < 10; i++)
	{
