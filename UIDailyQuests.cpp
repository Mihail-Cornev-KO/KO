...
#endif
#if (HOOK_SOURCE_VERSION == 1098)
		hunt[i] = Engine->GetChildByID(quest_rewards, "Hunt");
#else
		hunt[i] = Engine->GetChildByID(quest_rewards, string_format(xorstr("Hunt%d"), i + 1));
#endif
		str_hunt[i] = Engine->GetChildByID(hunt[i], "text_hunt_name");
		str_hunt_count[i] = Engine->GetChildByID(hunt[i], "text_hunt_count");
		hunt_area[i] = Engine->GetChildByID(hunt[i], "icon");
	}
	for (uint8 i = 0; i < 4; i++)
	{
...
#endif
	{
		Engine->SetString(str_hunt[i], "");
		Engine->SetString(str_hunt_count[i], "");

		if (auto mob = Engine->tblMgr->getMobData(quest->Mob[i]))
		{
			if (i == 0)
				Engine->SetString(str_quest, string_format(xorstr("%s"), mob->strName.c_str()));

			Engine->SetString(str_hunt[i], mob->strName);
		}
		else {
			if (quest->Mob[i] == 1) {
				if (i == 0)
				...
				Engine->SetString(str_hunt[i], xorstr("<Kill Enemy>"));
			}
			else {
				if (i == 0)
					Engine->SetString(str_quest, xorstr(""));

				Engine->SetString(str_hunt[i], xorstr(""));
			}
		}
		if (quest->Mob[i] > 0)
			Engine->SetString(str_hunt_count[i], string_format(xorstr("%d / %d"), quest->MyCount[i], quest->KillTarget[i]));
		...
		if (TABLE_ITEM_EXT* ext = Engine->tblMgr->getExtData(tbl->extNum, OriginItemID))
		{
			...
			Engine->SetTexImage(hunt_area[i], dxt);
		}
		else {
			...
			Engine->SetTexImage(hunt_area[i], szIconFN);
		}
	}
...
void HookQuestPage::KillTrigger(uint8 questID, uint16 mob)
{
	foreach(itr, kcbq_quests)
	{
		...
		{
			if ((*itr)->Mob[group] == mob && (*itr)->index == questID)
				(*itr)->MyCount[group]++;
		}

		if ((*itr)->MyCount[0] >= (*itr)->KillTarget[0] &&
			(*itr)->MyCount[1] >= (*itr)->KillTarget[1] &&
			(*itr)->MyCount[2] >= (*itr)->KillTarget[2] &&
			(*itr)->MyCount[3] >= (*itr)->KillTarget[3])
		{
			...
			{
				memset((*itr)->MyCount, 0, sizeof((*itr)->MyCount));
				(*itr)->remtime = 0;
...
#else
		if (filteredList[i]->strQuestName.c_str())
			Engine->SetString(txt_quest_title[j], string_format(xorstr("%s"), filteredList[i]->strQuestName.c_str()));
		else
			Engine->SetString(txt_quest_title[j], string_format(xorstr("%s"), "Kill Enemy"));

		if (filteredList[i]->premiumstatus)
			Engine->SetStringColor(txt_quest_title[j], D3DCOLOR_ARGB(255, 0, 255, 0));
#endif
