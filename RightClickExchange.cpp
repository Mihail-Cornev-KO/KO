...
const DWORD KO_INVENTORY_ITEM_MOVE = 0x0060E805;
...
void RightClickExchange::RightClickExchangePacket(Packet pkt)
{
	...
	case RightClickExchangeSubcode::GetExchangeMessage:
	{
		...
		if (bSucces)
		{
			Engine->WriteInfoMessage((char*)xorstr("RightClick Exchange successfully."), 0x009700);
			this->Close();
		}
		else if (!bSucces)
			Engine->WriteInfoMessage((char*)xorstr("RightClick Exchange failed."), 0xBA0F30);
