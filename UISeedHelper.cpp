...
{
	oCameraZoom = (typeJMP)DetourFunction((PBYTE)0x00A93CB9, (PBYTE)hkCameraZoom); // 2369
	...
	DWORD Adress = (*(DWORD*)0xF50FD8) + 0x1C8;
...
bool CUISeedHelperPlug::ReceiveMessage(DWORD* pSender, uint32_t dwMsg)
{
	...
	else if (m_btnSaveSettings != NULL && pSender == (DWORD*)m_btnSaveSettings)
	{
		...
		if (m_bNeedToSave)
		{
			...
			m_bNeedToSave = false;
			if (isRenderObject)*(uint8*)0x00476DE7 = 0x74; else*(uint8*)0x00476DE7 = 0x75;
		}
	}
