...
void CUISearchMonsterDropPlug::ParseUIElements()
{
	...
	auto basePercent = Engine->GetChildByID(m_dVTableAddr, xorstr("grp_percents"));
	...
	for (uint8 i = 0; i < 25; i++) {
		...
		dropBase[i]->itemID = 0;
		dropBase[i]->strPercent = Engine->GetChildByID(basePercent, string_format("percent_%d", i + 1));
		if (dropBase[i]->strPercent)Engine->SetVisible(dropBase[i]->strPercent, false);
	}
...
void CUISearchMonsterDropPlug::Order(uint8 page)
{
	for (auto& it : dropBase)
	{
		...
		Engine->SetVisible(it->strPercent, false);
	...
	for (auto base : dropBase)
	{
		...
		if (base)
		{
			...
			float per = 0.0f;
			if (myItems[i].sPercent > 0 && myItems[i].nItemID != 900000000)
				per = (myItems[i].sPercent / static_cast<float>(10000)) * 100;
			else if (myItems[i].nItemID == 900000000)
				per = 100.0f;

			if (per > 0.0f) // Drop Oranlarý sadece GM karakterlerde görünür.
			{
				/*Engine->SetString(base->strPercent, string_format("%.2lf%%", per));
				Engine->SetVisible(base->strPercent, true);*/
				if (Engine)
				{
					float per = (float)((float)myItems[i].sPercent / (float)100) > 100 ? 100 : (float)((float)myItems[i].sPercent / (float)100);
					Engine->SetVisible(base->strPercent, true);
					Engine->SetString(base->strPercent, string_format(xorstr("%.2lf%%"), per));
				}
			}
		...
		nope:
			if (base) {
				...
				Engine->SetVisible(base->strPercent, false);
			}
		}
		i++;
	}
