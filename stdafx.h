...
struct COLLECTION_RACE_EVENT
{
	...
	uint8 isUsed[10];
	uint8 isRandom[10];// 10 lu yapým 17.05.2024 end
	uint8 RandomID[10];// 10 lu yapým 17.05.2024 end
	uint8 RandomSlotID[10];// 10 lu yapým 17.05.2024 end
	std::string EventName;
	uint8 sZone;

	COLLECTION_RACE_EVENT()
	{
		// cr ödül arttýrmasý  06.05.20204
		for (int i = 0; i < 3; i++)
		{
		...
		memset(isRandom, 0, sizeof(isRandom));// 10 lu yapým 17.05.2024 end
		memset(RandomID, 0, sizeof(RandomID));// 10 lu yapým 17.05.2024 end
		memset(RandomSlotID, 0, sizeof(RandomSlotID));// 10 lu yapým 17.05.2024 end

		memset(isUsed, 0, sizeof(isUsed));
		// cr ödül arttýrmasý  06.05.20204 end
...
#define ANTICHEAT_MODE 1	// Anti Cheat korumasýnýn aktif olmasý için "1" , Pasif olmasý için "0" yapýlmasý gerekiyor.
#define HOOK_SOURCE_VERSION 2369	// 1098 Myk0, 1534 , 2369 Son version'dur.
#define DELETE_CAPTURE_FILE1 "./DeaFSoft/re_a.dfsoft"
#define DELETE_CAPTURE_FILE2 L"./DeaFSoft/re_a.dfsoft"
...
const	DWORD KO_FNC_END_GAME = 0x00A9C111;				// 2369 Oyundan hýzlý çýkma
const	DWORD KO_CALL_END_GAME = 0x004DC690;				// 2369  Oyundan hýzlý çýkma
