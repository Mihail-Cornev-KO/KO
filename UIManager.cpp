...
void CUIManager::Init(LPDIRECT3DDEVICE9 lDevice)
{
	...
	char buff[50];
	sprintf_s(buff, "DeaFSoft 2369 Client[%d]", GetCurrentProcessId());
...
void PearlEngine::OpenjuraidScreen(bool update, uint16 karus, uint16 elmo, uint16 remtime)
{
	...
	if (!m_UiMgr->uiJuraid) {
		std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\re_war_juraid_enter.dfsoft"));
...
void CUIManager::OpenSlaveMerchant()
{
	...
	{
		std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\slavemerchant.dfsoft"));
...
void CUIManager::OpenSlaveMerchantPrice()
{
	...
	{
		std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\price.dfsoft"));
...
void CUIManager::OpenSlaveAccept()
{
	...
	{
		std::string name = Engine->dcpUIF(xorstr("DeaFSoft\\checkslave.dfsoft"));
