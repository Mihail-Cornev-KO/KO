...
extern CSpell* GetSkillBase(int iSkillID);//target sc new end
...
void CUITargetBarPlug::ParseUIElements()
{
	...
	//target sc new
	skill_group = Engine->GetChildByID(m_dVTableAddr, xorstr("skill_group"));
	for (uint8 i = 0; i < 20; i++) {
		icon_skill1_[i] = Engine->GetChildByID(skill_group, string_format(xorstr("target_skillicon%d"), i));
	}
	//target sc new end
...
//target sc new
void CUITargetBarPlug::Update(Packet& pkt)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		if (spell)
		{
			...
			{
				Engine->SetTexImage(icon_skill1_[i], szIconFN);
				Engine->SetVisible(icon_skill1_[i], true);
		...
		{
			Engine->SetVisible(icon_skill1_[i], false);
		}
	}
}
//target sc new end
